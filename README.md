...
- Google Sheet 只有助手3可以修改；選品與文案助手只整理並交接待寫入資料，不直接更動 Sheet。
- 不再建立 Google Sheet 備份分頁。名稱含「備份」的舊分頁只有在使用者授權下才可清理；本文件不代表清理已經完成。
- 同一天已選取或已入庫的資料不得再次覆蓋。若發現相同日期或 `TH-YYYYMMDD` 格式的同日 ID，助手3必須停止操作並回報。
- 只有資料屬於新日期、且主代理明確核准時，才可替換上一輪尚未發布的資料。已發布、已完成或歷史資料一律不得覆寫。
- 助手3應優先使用使用者目前開啟的 Sheet 畫面驗證操作，不為額外精確驗證重複請求 CSV 網路權限。只有遇到登入、CAPTCHA、權限問題或目標不明時，才需要請使用者介入。
- 上述限制規範的是 Sheet 寫入與驗證流程，不會停用 Dashboard 的資料讀取；Dashboard 仍可透過既有公開 CSV 來源唯讀載入商品池與發文表。
- Git 寫入只有助手4可以執行，包括 `git add`、commit 與 push；每次仍須取得主代理對檔案範圍與操作的明確核准。
