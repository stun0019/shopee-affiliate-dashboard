...
- Google Sheet 只有助手3可以修改。選品與文案助手只交接商品、文案與狀態資料，不直接寫入 Sheet。
- 不再建立名稱含「備份」的新備份分頁；既有同類分頁只有在使用者授權下才可清理。本政策不表示舊分頁已經完成刪除。
- 寫入前必須檢查日期與 ID。同一天已選取或已入庫的資料不得再次覆蓋；若已有相同日期或 `TH-YYYYMMDD` 格式的同日 ID，立即停止並回報。
- 只有新日期資料且主代理已核准時，才可替換上一輪未發布資料。已發布、已完成與歷史資料不得覆寫。
- 操作驗證優先使用使用者目前開啟的 Sheet 畫面；不要只為額外精確驗證而重複請求 CSV 網路權限。只有登入、CAPTCHA、權限問題或目標不明時才請使用者介入。
- Dashboard 讀取與 Sheet 寫入是不同權限邊界：Dashboard 仍可透過既有公開 CSV 來源唯讀載入資料，本政策並未禁用 CSV。
- Git 寫入只有助手4可以執行；`git add`、commit 與 push 均須先取得主代理對精確檔案範圍的核准。
